...
const Config = z.object({
	...
	minScore: z.number().min(0).max(1).default(.3),
		...
		child.on("close", (code) => {
			...
			try {
				...
				if (parsed.schema_version !== SCHEMA_VERSION) {
					resolve({
						ok: false,
						error: `unsupported retrieval schema_version ${String(parsed.schema_version)} (expected ${SCHEMA_VERSION}) — Auto_code_audit and this plugin are out of contract`
					});
					return;
				}
				resolve({
...
function apply(ctx, config) {
	ctx.tools.register({
		name: "reuse_check",
		description: "Pre-write reuse firewall (Python repositories): BEFORE writing a new helper/service/manager, describe what you are about to implement and the existing codebase root — the plugin deterministically surfaces the existing Python implementations that already cover that intent (no LLM involved). Describe in ENGLISH keywords that a function name/docstring would use (e.g. \"load json config settings environment env override\" — Chinese-only descriptions match poorly against English code). Decide whether to REUSE, extract a shared component, or write new code. The result is advisory evidence, not a verdict: never delete or rewrite code based on retrieval alone. Call this before writing any new function when a similar capability may already exist.",
		parameters: {
			type: "object",
			properties: {
				description: {
		...
		output: {
			...
			render: (_args, value) => {
				...
				const lines = value.candidates.map((c) => {
					const ch = c.channels;
					const why = ch === void 0 ? "" : ` (name=${(ch.name ?? 0).toFixed(2)} doc=${(ch.lexical ?? 0).toFixed(2)} literal=${(ch.string ?? 0).toFixed(2)})`;
					const lock = c.locked ? ` 🔒 LOCKED${c.locked_by?.length ? ` by ${c.locked_by.join(", ")}` : ""}` : "";
		...
		async execute(args) {
			...
			return {
				...
				candidates: outcome.results.map((r) => ({
					...
					path: r.path,
					score: r.score,
					...r.channels === void 0 ? {} : { channels: r.channels },
					...r.doc_first === void 0 ? {} : { doc_first: r.doc_first },
					...r.locked === void 0 ? {} : { locked: r.locked },
					...r.locked_by === void 0 ? {} : { locked_by: r.locked_by }
				}))
			};
		}
...
//#endregion
export { Config, apply, inject, name, runRetrieval };
